...
function collectImages(data) {
  ...
  let images = getItemsOfType(data, "attachment")
    ...
    .filter(attachment =>
      /\.(gif|jpg|png)$/i.test(attachment.attachment_url[0])
    )
    .map(attachment => ({
...
function addContentImages(data, images) {
  ...
  getItemsOfType(data, "post").forEach(post => {
    ...
    while ((match = regex.exec(postContent)) !== null) {
      ...
      let exists = images.some(
        image => image.postId === postId && image.url === url
      );
      if (!exists) {
        images.push({
          id: -1,
...
function collectPosts(data) {
  ...
  return getItemsOfType(data, "post").map((post, index) => {
    // console.log(post);
...
function initTurndownService() {
  ...
  turndownService.addRule("iframe", {
    filter: "iframe",
    replacement: (content, node) => {
      let html = node.outerHTML.replace(
        'allowfullscreen=""',
        "allowfullscreen"
      );
      return "\n\n" + html + "\n\n";
...
function getPostCoverImageId(post) {
  ...
  let postmeta = post.postmeta.find(
 ...
 * @param {*} v
 */
function frontmatterValueProcess(v) {
  if (typeof v === "string") {
...
function writeMarkdownFile(post, postDir) {
  const frontmatter = Object.entries(post.frontmatter).reduce(
    (accumulator, pair) => {
      // return accumulator + pair[0] + ':' + ' "' + pair[1] + '"\n';
      return (
        accumulator + pair[0] + ":" + frontmatterValueProcess(pair[1]) + "\n"
      );
    },
    ""
  );
...
function getPostDir(post) {
  ...
  if (argv.postfolders) {
    ...
    let folder = decodeURIComponent(post.meta.slug);
    if (argv.prefixdate) {
